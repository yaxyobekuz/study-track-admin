...
const LeadDirectionChart = ({ dateParams }) => {
  const params = buildParams(dateParams);
  const queryKey = JSON.stringify(params);

  const { data } = useQuery({
    queryKey: ["leads", "directions-analytics", queryKey],
    queryFn: () => leadsAPI.getDirectionAnalytics(params).then((res) => res.data.data),
  });
  ...
  return (
    <Card title="Yo'nalishlar bo'yicha lidlar" className="space-y-4">
      {items.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left py-2 px-2">Yo'nalish</th>
                  ...
                  <tr key={item._id} className="border-b border-gray-50 hover:bg-gray-50/50">
                    <td className="py-2 px-2">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <span className="font-medium text-gray-800">{item.directionName}</span>
                      </div>
                    </td>
...
export default LeadDirectionChart;
