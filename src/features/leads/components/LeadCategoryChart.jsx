...
const LeadCategoryChart = ({ dateParams }) => {
  const params = buildParams(dateParams);
  const queryKey = JSON.stringify(params);

  const { data } = useQuery({
    queryKey: ["leads", "categories-analytics", queryKey],
    queryFn: () => leadsAPI.getCategoryAnalytics(params).then((res) => res.data.data),
  });
  ...
  return (
    <Card title="Toifalar bo'yicha lidlar" className="space-y-4">
      {items.length === 0 ? (
...
export default LeadCategoryChart;
