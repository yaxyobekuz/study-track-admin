...
// Data & mutations
import { REQUEST_TYPE_LABELS, SALARY_REQUEST_HINT } from "../data/payroll.data";
import { useReviewSalaryRequest } from "../queries/payroll.mutations";

/**
 * Oylik so'rovini ko'rib chiqish oynasi.
 * `openModal("reviewSalaryRequest", { request })`
 *
 * ⚠️ Tasdiq oylikni O'ZGARTIRMAYDI — faqat so'rov holatini yangilaydi.
 */
const SalaryRequestReviewModal = () => (
  <ResponsiveModal name="reviewSalaryRequest" title="Oylik so'rovini ko'rib chiqish">
    <Content />
  </ResponsiveModal>
);
...
const Content = ({ close, isLoading, setIsLoading, request }) => {
  const { mutate: review } = useReviewSalaryRequest();
  ...
  const hasProposal =
    request.proposedAmount || request.proposedHourlyRate || request.proposedStartMonth;
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-gray-100 px-3">
        ...
        {request.proposedAmount && (
          <Row label="Taklif (summa)">{formatMoney(request.proposedAmount)}</Row>
        )}
        {request.proposedHourlyRate && (
          <Row label="Taklif (soat)">{formatMoney(request.proposedHourlyRate)}</Row>
        )}
        {request.proposedStartMonthLabel && (
          <Row label="Qaysi oydan">{request.proposedStartMonthLabel}</Row>
        )}
        {!hasProposal && !request.reason && (
          <Row label="Tafsilot">—</Row>
        )}
      </div>
...
export default SalaryRequestReviewModal;
