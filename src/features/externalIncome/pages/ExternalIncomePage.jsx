...
/**
 * TASHQI KIRIMLAR — o'quvchi to'lovi BO'LMAGAN pul: ijara, kitob sotuvi,
 * homiylik.
 *
...
const ExternalIncomePage = () => {
  ...
  const tabs = INCOME_TABS.filter(
    (item) => item.value !== "settings" || can("income.categories"),
  ).map((item) => ({
    ...item,
    content: item.value === "main" ? <IncomeList /> : <CategoriesPanel />,
  }));
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Can do="income.create">
          <Button onClick={() => openModal("externalIncome", {})}>
            <Plus />
            Kirim qo'shish
          </Button>
        </Can>
      ...
      {data?.totals && (
        <Card>
          <p className="text-xs font-medium text-gray-500">
            {isFiltered ? "Tanlangan filtr bo'yicha" : "Jami tashqi kirim"}
          </p>
      ...
      {isLoading ? (
        ...
        <Card className="p-0 xs:p-0">
          <EmptyState
            icon={Coins}
            title={isFiltered ? "Yozuv topilmadi" : "Tashqi kirim yo'q"}
            description={
              isFiltered
        ...
        <>
          <Table columns={INCOME_TABLE_COLUMNS}>
            {items.map((income) => (
              <Tr key={income.id} className={cn(income.isVoided && "opacity-50")}>
                <Td className="text-gray-500">{formatDateUz(income.occurredAt)}</Td>
                  ...
                  <div className="flex items-center justify-end">
                    {income.isVoided ? (
                      ...
                      <Can do="income.void">
                        <button
                          title="Bekor qilish"
                          onClick={() => openModal("voidExternalIncome", { income })}
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button onClick={() => openModal("incomeCategory", {})}>
          <Plus />
          Kategoriya qo'shish
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <Card className="p-0 xs:p-0">
          <EmptyState
            ...
            description="Kirim manbalarini kategoriyalarga ajrating: Ijara, Kitob sotuvi, Homiylik."
            action={
              <Button onClick={() => openModal("incomeCategory", {})}>
                <Plus />
                Kategoriya qo'shish
              ...
              <Tr key={category.id}>
                ...
                <Td>
                  <div className="flex items-center justify-end gap-1">
                    <button
                      title="Tahrirlash"
                      onClick={() => openModal("incomeCategory", { category })}
